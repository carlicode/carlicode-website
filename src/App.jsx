  ...
  return <>
    ...
    <main>
      <section id="inicio" className="hero-shell">
        ...
        <div className="hero-copy"><p className="eyebrow"><span /> CREADORA · COMMUNITY BUILDER</p><h1>Ideas que<br /><em>encienden</em><br />comunidad.</h1><p className="hero-lede">Soy Carli. Creo contenido y conversaciones que hacen que la tecnología, las ideas y las oportunidades se sientan más cerca.</p><div className="hero-actions"><a className="button button-primary" href="#comunidad">Conoce mi universo <ArrowUpRight size={18} /></a><a className="text-link" href="#colabora">Trabajemos juntas <span>↓</span></a></div></div>
        <div className="hero-stamp"><span>01</span><span>CARLI<br />CODE</span></div><div className="scroll-cue">SCROLL TO EXPLORE <i /></div>
      </section>
      <section id="comunidad" className="section community-section">
        <div className="section-intro split-intro"><div><p className="eyebrow"><span /> MI LUGAR FAVORITO</p><h2>Crear no es hablar <em>sola.</em></h2></div><p>Es encontrarnos. Mi contenido vive entre preguntas, herramientas digitales, aprendizajes imperfectos y la energía de una comunidad que se mueve.</p></div>
      ...
      <section id="trayectoria" className="section story-section">
        ...
        <div className="story-copy"><p className="eyebrow"><span /> MÁS QUE UN FEED</p><h2>Una trayectoria hecha de <em>curiosidad.</em></h2><p>Vengo del mundo tech, pero mi motor siempre ha sido traducir lo complejo en algo útil, humano y posible. Hoy esa curiosidad toma forma en contenido, escenarios y proyectos con propósito.</p><div className="signal-list"><div><Sparkles size={19} /><span><b>Ideas</b> que se comparten</span></div><div><Users size={19} /><span><b>Personas</b> que se encuentran</span></div><div><Mic2 size={19} /><span><b>Historias</b> que se amplifican</span></div></div><a href="#colabora" className="text-link">Conoce mi trabajo <ArrowUpRight size={17} /></a></div>
      </section>
      <section className="section pulse-section"><div className="pulse-orbit orbit-one" /><div className="pulse-orbit orbit-two" /><p className="eyebrow"><span /> EL EFECTO CARLI</p><h2>Una idea puede ser el<br /><em>comienzo de algo enorme.</em></h2><p>De una conversación a una comunidad. De una duda a una nueva posibilidad.</p><a className="play-button" href="#colabora"><Play fill="currentColor" size={18} /> HABLEMOS</a></section>
      <section id="archivo" className="section archive-section"><div className="archive-heading"><div><p className="eyebrow"><span /> TODO EN UN SOLO LUGAR</p><h2>Mi <em>archivo vivo.</em></h2></div><p>Charlas, proyectos, perfiles y enlaces para conocer las diferentes facetas de mi trabajo.</p></div><div className="archive-controls" aria-label="Filtrar archivo">{["Todo", "Charlas", "Proyectos", "Perfiles", "Trayectoria"].map((category) => <button key={category} className={activeCategory === category ? "active" : ""} onClick={() => selectCategory(category)}>{category}</button>)}</div><div className="archive-grid">{pageItems.map((item) => <article className="archive-card" key={`${item.category}-${item.title}`}><div className="archive-card-meta"><span>{item.category}</span><span>0{archiveItems.indexOf(item) + 1}</span></div><h3>{item.title}</h3><p>{item.description}</p>{item.href ? <a href={item.href} target="_blank" rel="noreferrer">Abrir enlace <ExternalLink size={16} /></a> : <span className="archive-no-link">Proyecto de portafolio</span>}</article>)}</div><nav className="pagination" aria-label="Paginación del archivo"><button onClick={() => setPage((current) => Math.max(1, current - 1))} disabled={page === 1}>← Anterior</button>{Array.from({ length: totalPages }, (_, index) => <button key={index + 1} className={page === index + 1 ? "active" : ""} onClick={() => setPage(index + 1)} aria-current={page === index + 1 ? "page" : undefined}>{String(index + 1).padStart(2, "0")}</button>)}<button onClick={() => setPage((current) => Math.min(totalPages, current + 1))} disabled={page === totalPages}>Siguiente →</button></nav></section>
      <section id="colabora" className="section contact-section"><div><p className="eyebrow"><span /> COLABORACIONES</p><h2>¿Creamos algo<br /><em>que importe?</em></h2></div><div className="contact-side"><p>Para campañas, charlas, proyectos de comunidad o una buena conversación.</p><a className="contact-email" href="mailto:hola@carlicode.com">hola@carlicode.com <ArrowUpRight size={22} /></a></div></section>
    </main>
